    ...
    <Tab.Navigator screenOptions={screenOptions} initialRouteName='Home'>
      <Tab.Screen name='Tasks' component={Tasks} options={{
        tabBarIcon: ({ focused }) => (
          <View style={{ alignItems: 'center', justifyContent: 'center' }}>
            <View style={[styles.icones, { backgroundColor: focused ? '#8AC600' : null }]}>
              <Feather name="check-square" size={24} color={focused ? '#ffff' : '#5C4B4B'} />
            </View>
          </View>
        )
      }} />
      <Tab.Screen name='Câmera' component={Camera} options={{
        tabBarIcon: ({ focused }) => (
          <View style={{ alignItems: 'center', justifyContent: 'center' }}>
            <View style={[styles.icones, { backgroundColor: focused ? '#8AC600' : null }]}>
              <Feather name='camera' size={24} color={focused ? '#ffff' : '#5C4B4B'} />
            </View>
          </View>
        )
      }} />
      <Tab.Screen name='Home' component={Home} options={{
        tabBarIcon: ({ focused }) => (
          <View style={{ alignItems: 'center', justifyContent: 'center' }}>
            <View style={[styles.icones, { backgroundColor: focused ? '#8AC600' : null }]}>
              <AntDesign name="home" size={30} color={focused ? '#ffff' : '#5C4B4B'} />
            </View>
          </View>
        )
      }} />
      <Tab.Screen name='Exercício' component={Exercicios} options={{
        tabBarIcon: ({ focused }) => (
          <View style={{ alignItems: 'center', justifyContent: 'center' }}>
            <View style={[styles.icones, { backgroundColor: focused ? '#8AC600' : null }]}>
              <FontAwesome5 name="running" size={24} color={focused ? '#ffff' : '#5C4B4B'} />
            </View>
          </View>
        )
      }} />
      <Tab.Screen name='Configurações' component={Config} options={{
        tabBarIcon: ({ focused }) => (
          <View style={{ alignItems: 'center', justifyContent: 'center' }}>
            <View style={[styles.icones, { backgroundColor: focused ? '#8AC600' : null }]}>
              <Feather name="settings" size={24} color={focused ? '#ffff' : '#5C4B4B'} />
            </View>
          </View>
        )
      }} />
  ...
  return (
    <Stack.Navigator>
      ...
      <Stack.Screen name="Perfil" component={Perfil} options={{
          ...
          headerTitle: () => (
            <View>
              <Text style={{ color: '#5C4B4B', fontSize: 25, fontFamily: 'Poppins_700Bold', alignItems: 'center', justifyContent: 'center', marginTop: '10%' }}>Perfil</Text>
            </View>
          ),
          headerTitleStyle: {
