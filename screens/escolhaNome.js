...
export default function EscolhaNome({ route, navigation }) {
	...
	const schema = yup.object({
		nomeBicho: yup
			.string()
			.required('É obrigatório dar um nome para seu novo amigo!'),
	});
		...
		<ScrollView contentContainerStyle={styles.scrollContainer}>
			<View style={styles.container}>
				...
				<View style={styles.erro}>
					{errors.nomeBicho && (
						<Text style={styles.erroMensagem}>{errors.nomeBicho?.message}</Text>
					)}
				</View>
				<TouchableOpacity
					style={styles.btnIniciarJogo}
					onPress={handleSubmit(verificaAmigo)}
				>
